...
#         mask_name = self.filenames[idx].replace("patient_", "segmentation_")
...
#             mask = augmented["mask"]
#         else:
#             # Fallback: normalize and convert to tensor
...
#     return A.Compose([
#         A.Resize(256, 256),               # <-- Resize to fixed size
...
#     return A.Compose([
#         A.Resize(256, 256),               # <-- Resize to fixed size for validation too
#         A.Normalize(),
#         ToTensorV2()
#     ])
...
# ### Train Function ###
# def train(model, train_loader, val_img_dir, val_mask_dir, epochs=10, lr=1e-3):
#     model.to(device)
...
#     validate(model, val_img_dir, val_mask_dir)

# ### Validate ###
# def validate(model, image_dir, mask_dir):
#     model.eval()
#     model.to(device)
...
#         mask_name = fname.replace("patient_", "segmentation_")
...
#         img = aug(image=img)["image"].unsqueeze(0).to(device)

#         with torch.no_grad():
#             output = torch.sigmoid(model(img)).cpu().squeeze().numpy()

#         pred_mask = cv2.resize(output, (original_shape[1], original_shape[0]))
...
#     tumor_avg = np.mean(tumor_positive_scores) if tumor_positive_scores else 0.0
#     tumor_sum = np.sum(tumor_positive_scores) if tumor_positive_scores else 0.0

#     print(f"\nAverage Dice Score on All Cases: {avg_dice:.4f}")
#     print(f"Tumor-Positive Case Average: {tumor_avg:.4f}")
#     print(f"Tumor-Positive Case Total: {tumor_sum:.2f}")
#     print(f"Format for leaderboard: {tumor_sum:.2f} ({tumor_avg:.2f})")

# ### Run ###
# if __name__ == "__main__":
#     train_dataset = TumorDataset("data/train/imgs", "data/train/labels", transform=get_train_transform())
#     train_loader = DataLoader(train_dataset, batch_size=4, shuffle=True)

#     model = get_unet_model()
#     train(model, train_loader, "data/val/imgs", "data/val/labels", epochs=10, lr=1e-4)
# Format for leaderboard: 23.46 (0.63)
...
#Format for leaderboard: 27.60 (0.75)